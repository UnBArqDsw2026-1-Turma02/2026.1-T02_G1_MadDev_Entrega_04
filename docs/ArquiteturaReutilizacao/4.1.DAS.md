...
> Esta seção **complementa** o documento (nada abaixo foi removido) e registra a **evolução do projeto** entre as entregas anteriores e a Entrega Final, conforme o estado atual da implementação na branch `game` (Godot 4.6). Onde houver divergência entre o texto original e esta atualização, **prevalece esta seção**.

**Estado geral:** toda a lógica de jogo está implementada e validada em `godot --headless` (loop completo: menu → seleção de personagem → 12 salas em sequência → morte → tela de fim → reset). Restam apenas **assets** (sprites, música e SFX) e playtests manuais.

| Tópico | No texto original | Estado na Entrega Final |
|--------|-------------------|-------------------------|
| **Resolução** (§3, §10) | 320×180 lógico | **640×360** (16:9; janela 1280×720; `stretch=canvas_items`, `aspect=keep`) |
| **Boss** (§4, §10) | "previsto" / não implementado | **Implementado** (`enemy_boss.gd` + `enemy_boss.tscn`): **2 fases**, gatilho a 50% de HP, multiplicadores ×1.6 velocidade / ×1.5 dano |
| **Tipos de inimigo** (§10) | 2 (melee, ranged) | **3** (melee, ranged, **boss**) |
| **Atributos do Player** (§5.2) | `defense`, `resistance`, `dash_cooldown` | Consolidados em **`PlayerStats`** (Resource): vida em **corações (base 3)**, dano base 10, velocidade base 200 e **redução de cooldown de dash**. **Dash por cargas** (2 cargas, recarga 1.6 s/carga) |
| **Consumíveis** (§5.2 `items`) | stub (sem classe concreta) | **Concretos:** Bomba, Chave, Poção, Status (+1 atributo/+1 XP) e Benefício (slot único) |
| **Raridade** (§5.1/§5.2) | apenas `RareDecorator` | **`RarityConfig`** completo — Comum/Incomum/Raro/Épico + cores (Flyweight) |
| **Economia & Loja** (ausente) | — | **Moeda** (`GameManager.currency`), **4 NPCs vendedores** (Armeiro/Equipador/Boticário/Mentor), **baús** (chave/bomba) e **recompensas de sala** (item 15 / arma 25 / vitória 100) |
| **Sequência da run** (§4, §10) | "sala 11 pré-boss" | Sequência real (12 salas): `empty, combat, combat, rest, combat, chest, combat, shop, rest, combat, combat, boss` |
| **Dimensões do código** (§10) | ~35 scripts, ~10 cenas | **81 scripts, 27 cenas, 14 resources** |
| **Padrões GoF** (§5.1) | tabela de 17 padrões | acrescentar **Flyweight** (`rarity_config.gd`, `enemy_data.gd`, `item_data.gd`, `weapon_data.gd`) |

### Nota sobre `GameMediator` × `SignalBus`

O documento descreve um **barramento de eventos duplo** (`GameMediator` + `SignalBus`), mantido aqui como **conceito de arquitetura idealizado**. Na implementação atual, essa coordenação de eventos foi **consolidada no `SignalBus`** — os 6 autoloads registrados são `SignalBus`, `GameManager`, `AudioManager`, `GameFacade`, `AchievementManager` e `StudentProfileRegistry` (não há um autoload `GameMediator` separado; o `SignalBus` atua como mediador central). Os diagramas que citam o `GameMediator` permanecem válidos como representação do **papel de mediação**, exercido pelo `SignalBus` no código.

> **Pendências de produção (não-código):** sprites de player/inimigos/itens, texturas, trilha sonora e efeitos sonoros (o `AudioManager` já integra; faltam os arquivos de mídia).
...
graph LR
    ...
    J --> UC1["Abrir jogo / Navegar menu<br/>/ Iniciar partida<br/>(US-48)"]
    J --> UC2["Pausar e Retomar Partida<br/>(US-49, US-50)"]
    J --> UC3["Sistema de Combate<br/>(US-21, US-22)"]
    J --> UC4["Vida, Cura e Morte<br/>(US-18, US-19, US-20)"]
    J --> UC5["Uso de Consumíveis<br/>(US-34, US-35, US-36)"]
    J --> UC6["Equipar Itens<br/>(US-37, US-38, US-39)"]
    J --> UC7["Inventário e Descarte<br/>(US-51, US-52)"]
    I --> UC8["Comportamento dos Inimigos<br/>(US-30, US-31, US-32, US-33)"]
```
...
```mermaid
graph TD
    UI["📦 ui<br/>main_menu · hud_abstraction<br/>pause_menu · renderers/"]
    GP["📦 gameplay<br/>player · enemies · projectiles<br/>items · consumables · world"]
    SVC["📦 autoloads — Serviços Globais<br/>GameFacade · GameManager<br/>GameMediator · SignalBus<br/>AudioManager · AchievementManager<br/>StudentProfileRegistry"]
    DATA["📦 resources — Dados<br/>StudentProfile · Achievement<br/>DifficultyConfig · .tres"]
...
A tabela abaixo cruza cada padrão GoF implementado no projeto com a camada em que atua e o problema arquitetural que resolve:

| Padrão GoF | Categoria | Camada | Problema Arquitetural Resolvido |
|------------|-----------|--------|----------------------------------|
| Singleton | Criacional | Serviços (Autoloads) | Garantir instância global única de serviços como `GameManager` e `AudioManager`, acessível sem instanciação manual |
| Factory Method | Criacional | Gameplay | Criar inimigos sem acoplar `EnemySpawner` ao tipo concreto instanciado; novos tipos são adicionados sem alterar o código de spawn |
| Builder + Director | Criacional | Gameplay | Construir salas `Node2D` passo a passo com API fluente; o `Director` encapsula receitas de sala sem depender da implementação do `Builder` |
| Multiton | Criacional | Serviços / Dados | Garantir instância única por chave de perfil em `StudentProfileRegistry`, evitando leituras duplicadas de disco |
| Object Pool | Criacional | Gameplay | Eliminar alocação dinâmica de projéteis durante combate, reduzindo pressão no GC do GDScript |
| Facade | Estrutural | Serviços (Autoloads) | Reduzir acoplamento entre UI e subsistemas; centralizar operações compostas como `kill_player()` em um único ponto de entrada |
| Bridge | Estrutural | UI | Separar a abstração do HUD (`HUDAbstraction`) da renderização concreta (`CanvasRenderer` / `DebugRenderer`), permitindo trocar o renderer sem alterar a lógica |
| Decorator | Estrutural | Gameplay | Adicionar efeitos a itens em runtime empilhando decoradores sem herança múltipla |
| Adapter | Estrutural | Gameplay | Isolar a leitura de input do jogador da lógica de movimento, facilitando troca de esquema de controle |
| Mediator | Comportamental | Serviços (Autoloads) | Eliminar acoplamento direto entre sistemas; `GameMediator` roteia eventos tipados de muitos-para-muitos sem que os sistemas se conheçam |
| Observer | Comportamental | Serviços (Autoloads) | `SignalBus` notifica sistemas interessados sobre mudanças de estado sem dependência direta entre remetente e ouvinte |
| Chain of Responsibility | Comportamental | Gameplay | Aplicar modificadores de dano em sequência configurável (`ArmorHandler` → `ResistanceHandler` → `HealthHandler`) de forma extensível |
| Strategy | Comportamental | Gameplay | Intercambiar comportamento de IA de inimigos (`EnemyMelee` persegue; `EnemyRanged` kita) sem alterar a lógica de spawn |
| Template Method | Comportamental | Gameplay | Definir o esqueleto do algoritmo de movimento/ataque em `EnemyBase` e `ConsumableBase`, com pontos de extensão em subclasses |
| Visitor | Comportamental | Gameplay | `RoomValidator` opera sobre `Door` para destrancá-la ao limpar a sala, sem modificar a classe visitada |
| Iterator | Comportamental | Gameplay | `SortedEnemyIterator` e `FilteredItemIterator` percorrem coleções com critérios customizados sem expor a estrutura interna |
| Command | Comportamental | Gameplay | `_read_dash_input()` e `_execute_dash()` no `Player` encapsulam leitura de input e execução de ação como operações separadas |
...
classDiagram
    ...
    class DoubleDropDecorator {
        +get_effect() String
        +get_value() int
    }
    class SortedEnemyIterator {
        +enemies: Array
        +origin: Vector2
        +_iter_init(arg) bool
        +_iter_next(arg) bool
...
O jogo executa como **processo único** (single-process) no loop de física e renderização do Godot 4.6. Não há threads secundárias; toda a lógica roda na main thread via `_process()` e `_physics_process()`. A comunicação assíncrona é feita por sinais Godot (event-driven, single-threaded) e pelo `GameMediator`.

### 6.1 Autoloads como Processos Persistentes

Os Autoloads são registrados no `project.godot` e instanciados pelo motor **antes de qualquer cena ser carregada**. Uma vez que seu `_ready()` é executado, eles permanecem ativos na `SceneTree` durante toda a sessão — inclusive durante trocas de cena —, funcionando como os processos persistentes da arquitetura orientada a eventos do Projeto.

| Autoload | Script | Responsabilidade Permanente | Persiste entre trocas de cena |
|----------|--------|-----------------------------|-------------------------------|
| `GameFacade` | `game_facade.gd` | Ponto de entrada único para ações de gameplay; orquestra subsistemas atomicamente | ✅ sim |
| `GameManager` | `game_manager.gd` | Mantém o estado da run (`GameState`, score, XP, nível, room index) e controla o game loop | ✅ sim |
| `GameMediator` | `game_mediator.gd` | Roteador de eventos tipados; mantém o `Dictionary` de `Callable` registrados por evento | ✅ sim |
| `SignalBus` | `signal_bus.gd` | Canal de sinais nativos Godot; permite conexões via inspetor e bridge para `GameMediator` | ✅ sim |
| `AudioManager` | `audio_manager.gd` | Controle global de música e efeitos sonoros, com carregamento de streams por nome | ✅ sim |
| `AchievementManager` | `achievement_manager.gd` | Rastreia estatísticas da run e avalia condições de desbloqueio de conquistas | ✅ sim |
| `StudentProfileRegistry` | `student_profile_registry.gd` | Cache de perfis `.tres`; garante que cada perfil seja lido do disco apenas uma vez | ✅ sim |

Os diagramas de sequência a seguir mostram esses processos persistentes interagindo durante eventos reais de gameplay.

### 6.2 Inicialização de Run
...
### 6.3 Loop de Combate e Liberação de Sala
...
### 6.5 Morte do Jogador e Permadeath
...
graph TD
    ...
    subgraph Maquina["Máquina do Jogador"]
        ...
        subgraph Motor["Motor"]
            ...
            GE["Godot 4.6 Engine<br/>(GL Compatibility)"]
            %% A GDScript VM interpreta e executa todo o código do jogo em tempo real
            VM["GDScript VM<br/>(main thread)"]
        end

        subgraph Recursos["Recursos Embutidos"]
...
O diagrama abaixo mostra as relações entre os Resources e o ciclo de vida dos dados durante uma sessão:

```mermaid
graph TD
    %% Ciclo de vida dos dados: do disco até o reset de permadeath (RNF-07)

    subgraph Disco["Disco — somente leitura, empacotado no executável"]
        P1["profiles/calouro.tres"]
        P2["profiles/veterano.tres"]
        P3["profiles/jubilado.tres"]
        P4["profiles/atletica.tres"]
        T1["art/tilesets/room.tres"]
        T2["art/themes/main_theme.tres"]
    end

    subgraph Memoria["Memória — carregado uma única vez no startup via _ready()"]
        REG["StudentProfileRegistry<br/>(Multiton — cache de perfis)"]
        RB["RoomBuilder<br/>(usa tileset)"]
        UI_THEME["Cenas de UI<br/>(usa tema visual)"]
    end

    subgraph Volatil["Estado Volátil da Run — existe apenas em memória durante a sessão"]
        VOL["vida atual · XP · nível<br/>inventário · score · sala atual"]
    end

    %% Resources de perfil são carregados no registry e ficam em cache
    P1 & P2 & P3 & P4 -->|"load() no _ready()"| REG
    %% Resources visuais são carregados pelos respectivos consumidores
    T1 -->|"load() no _ready()"| RB
    T2 -->|"load()"| UI_THEME

    %% No início de cada run, o perfil escolhido aplica seus modificadores ao jogador
    REG -->|"apply_to(player)<br/>no início da run"| VOL

    %% RNF-07: ao morrer, reset_run() descarta todo o estado volátil
    VOL -->|"morte ou vitória"| RESET["GameManager.reset_run()<br/>(RNF-07 — permadeath)"]
    RESET -->|"zera tudo"| VOL
```
...
|------|--------|-----------|-------|
| 21/06/2026 | 1.0 | Criação do documento | [Vinícius Rufino](https://github.com/RufinoVfR), [Mateus Vieira](https://github.com/matix0),  [Breno Lucena Cordeiro](https://github.com/BrenoLUCO) |
| 21/06/2026 | 1.1 | Revisão do documento baseando-se nas entregas passadas | [Vinícius Rufino](https://github.com/RufinoVfR) |
| 21/06/2026 | 1.2 | Documentação das Visões de Implementação e Lógica | [Vinícius Rufino](https://github.com/RufinoVfR) e [Breno Lucena Cordeiro](https://github.com/BrenoLUCO) |
| 21/06/2026 | 1.3 | Documentação do DAS por completo | [Vinícius Rufino](https://github.com/RufinoVfR) |
| 22/06/2026 | 1.4 | Adição de Sumário Executivo e Entrega do Módulo 4.1 | [Breno Lucena](https://github.com/BrenoLUCO) |
| 22/06/2026 | 1.5 | Seção "🔄 Atualização — Entrega Final": alinhamento com a implementação (branch `game`) — resolução 640×360, boss implementado, PlayerStats, consumíveis/raridade/economia/loja, sequência real de salas, Flyweight e nota sobre GameMediator/SignalBus | [Felipe Veríssimo](https://github.com/verissimoo) |

---

## Apêndice: Sumário Executivo — Módulo 4.1

### Introdução

Este apêndice entrega o Módulo 4.1 — Documento de Arquitetura de Software (DAS) solicitado na atividade. Ele contém duas visões: a Visão Lógica (template para preencher) e a Visão de Implementação (preenchida a partir do código existente no repositório). Também traz instruções para demonstrar o código rodando, referências e o quadro de participações (4.3).

### Escopo da Entrega

- Visão obrigatória: **Visão Lógica** ✓ (Seção 5)
- Visão escolhida: **Visão de Implementação** ✓ (Seção 8)
- Complemento: **Quadro de Participações (4.2)** (vide arquivo [4.2.ParticipacoesArqReutilizacao.md](4.2.ParticipacoesArqReutilizacao.md))
- Código comprobatório: trechos e cenas para demonstração no Godot

### Organização de Pacotes / Arquivos

Estrutura principal do código (extraída de `jogo/scripts`):

- **`autoloads/`** — `game_facade.gd`, `game_manager.gd`, `audio_manager.gd`, `achievement_manager.gd`, `signal_bus.gd`
  - Singletons / serviços globais / Facade

- **`enemies/`** — `enemy_factory.gd`, `enemy_base.gd`, `enemy_melee.gd`, `enemy_ranged.gd`, `enemy_spawner.gd`
  - Factory, spawners, IA

- **`items/`** — `item_base.gd`, `decorators/` (`item_decorator.gd`, `burn_decorator.gd`, `rare_decorator.gd`, `double_drop_decorator.gd`)
  - Decorator pattern

- **`world/`** — `room_builder.gd`, `room_director.gd`, `room_validator.gd`, `projectile_pool.gd`, `door.gd`, `run.gd`
  - Builder/Director, Object Pool

- **`ui/`** — `main_menu.gd`, `pause_menu.gd`, `hud_abstraction.gd`, `renderers/`
  - Telas e renderers

- **`damage/`** — `damage_handler.gd`, `health_handler.gd`, `armor_handler.gd`, `resistance_handler.gd`
  - Chain of Responsibility / Handlers

- **`iterators/`** — `filtered_item_iterator.gd`, `sorted_enemy_iterator.gd`
  - Iterator pattern

- **`resources/`** — configurações e registries (`student_profile.gd`, `difficulty_config.gd`, etc.)

### Dependências Principais

- `ui/` e `world/` dependem de `autoloads/` (GameFacade / GameManager / SignalBus)
- `world/` cria ou referencia `enemies/` e `items/` (via `EnemyFactory`, `RoomBuilder`)
- `player` e `enemies` usam `damage/` handlers para processar dano
- `ProjectilePool` (em `world/`) é utilizado por `player` e por cenas de projéteis

### Padrões Arquiteturais Identificados

| Padrão | Implementação | Benefício |
|--------|----------------|----------|
| **Singleton / Autoload** | `game_manager.gd`, `audio_manager.gd` (autoregistrados em `project.godot`) | Estado global garantido e acessível em qualquer lugar |
| **Facade** | `game_facade.gd` | Interface unificada para subsistemas |
| **Factory Method** | `enemy_factory.gd` | Ponto único de criação de inimigos, extensível |
| **Builder / Director** | `room_builder.gd`, `room_director.gd` | Construção stepwise de salas complexas |
| **Object Pool** | `projectile_pool.gd` | Reuso de instâncias de projéteis, evita thrashing do GC |
| **Decorator** | `item_decorator.gd` + `ItemBase` | Composição dinâmica de efeitos de item |
| **Iterator** | `filtered_item_iterator.gd`, `sorted_enemy_iterator.gd` | Iteração especializada sobre coleções |
| **Chain of Responsibility** | `damage/damage_handler.gd` e handlers específicos | Pipeline de transformação de dano configurável |
| **Template Method / Strategy** | `enemy_base.gd` | Template de ataque com hooks para subclasses |

### Justificativa da Organização

A separação em módulos por responsabilidade (autoloads, enemies, items, world, ui, damage) favorece manutenção e testabilidade. Padrões como Factory, Builder e Decorator aumentam a extensibilidade:

- Adicionar novo inimigo: afeta apenas `enemies/` e `enemy_spawner.gd`
- Adicionar novo tipo de sala: afeta apenas `world/` e `room_builder.gd`
- Adicionar novo efeito de item: afeta apenas `items/decorators/`

Resultado: **mudanças locais e baixo acoplamento**.
...
|----------|------|-----------|
| `move_speed` | float | Velocidade de movimento |
| `dash_speed` | float | Velocidade do dash |
| `max_health` | int | Vida máxima |
| `base_damage` | int | Dano base |
| `defense` | int | Defesa (redução flat) |
| `resistance` | float | Resistência (redução percentual) |
| `equipment` | Dictionary | Slots de equipamento |

**Métodos:** `take_damage()`, `apply_final_damage()`, `equip()`, `_shoot()`

**Dependências:** `ProjectilePool`, `DamageHandler`, `SignalBus`

#### EnemyBase (`enemy_base.gd`) + Subclasses
...
| `attack_damage` | Dano do ataque |
| `move_speed` | Velocidade de movimento |

**Métodos:** `take_damage()`, `attack_sequence()`, `_get_move_direction()`, `execute_attack()` (subclasses)

**Dependências:** `SignalBus`, `DamageHandler`

#### ItemBase (`item_base.gd`) / ItemDecorator (`item_decorator.gd`)

| Atributo | Descrição |
|----------|-----------|
| `item_name` | Nome do item |
| `base_value` | Valor base |
| `item_type` | Tipo (weapon, armor, etc.) |

**Métodos:** `get_effect()`, `get_value()`

**Uso:** Equipável pelo `Player`; decoradores empilham efeitos

#### RoomBuilder / RoomDirector

**Responsabilidade:** Construir a cena da sala com tiles, portas, inimigos e itens

#### ProjectilePool

**Responsabilidade:** Prover/recuperar projéteis sem instanciar/destruir constantemente

#### GameFacade

**Responsabilidade:** Interface simples para subsistemas (AudioManager, GameManager, SignalBus)

#### DamageHandler / Handlers

**Responsabilidade:** Pipeline que calcula dano final aplicado ao alvo

### Diagrama de Classes (Simplificado)

```mermaid
classDiagram
    Player "1" --> "*" ItemBase : equips
    Player ..> ProjectilePool : uses
    EnemyBase <|-- EnemyMelee
    EnemyBase <|-- EnemyRanged
    RoomBuilder --> EnemyFactory : creates
    ItemDecorator --|> ItemBase
    DamageHandler <|-- HealthHandler
    DamageHandler <|-- ArmorHandler
    DamageHandler <|-- ResistanceHandler
```

### Código Comprobatório — Passo a Passo para Demo

#### Arquivos/Cenas Úteis

- **Projeto Godot:** `jogo/project.godot`
- **Scenes de teste:** Verificar `jogo/scenes/` — há vários arquivos `test_*.tscn` e `test_patterns.tscn` que já exercitam padrões

#### Como Rodar

1. Abrir o projeto em Godot 4.6
2. Executar a cena principal (`main.tscn` ou similar) via F5
3. Ou executar testes isolados com F6 `(test_builder.tscn`, `test_facade.tscn`, etc.)

### Histórico de Versões

| Nome | Alteração | Versão | Data | Revisor | Data de Revisão | Status |
|------|-----------|--------|------|---------|-----------------|--------|
| [Breno Lucena](https://github.com/BrenoLUCO/) | Desenvolvimento | v0.1 | 22/06/2026 | - | - | Draft |

### Referências

- Padrões GoF — Factory, Builder, Decorator, Iterator, Object Pool
- [Documentação Godot Engine](https://docs.godotengine.org)
- Material do curso / Diretrizes da entrega (Aprender3)
| 21/06/2026 | 1.4 | Adição da Tabela de padrões GoF por camada, documentação dos autoloads como processos persistentes e diagrama na Visão de Dados. | [Vinícius Rufino](https://github.com/RufinoVfR) |