...
graph TD
    ...
    subgraph Disco — somente leitura, empacotado no executável
    ...
    subgraph Memória — carregado uma única vez no startup via _ready()
        REG["StudentProfileRegistry\n(Multiton — cache de perfis)"]
        RB["RoomBuilder\n(usa tileset)"]
        UI_THEME["Cenas de UI\n(usa tema visual)"]
    end

    subgraph Estado Volátil da Run — existe apenas em memória durante a sessão
        VOL["vida atual · XP · nível\ninventário · score · sala atual"]
    end
    ...
    REG -->|"apply_to(player)\nno início da run"| VOL

    %% RNF-07: ao morrer, reset_run() descarta todo o estado volátil
    VOL -->|"morte ou vitória"| RESET["GameManager.reset_run()\n(RNF-07 — permadeath)"]
    RESET -->|"zera tudo"| VOL
```
...
- Complemento: **Quadro de Participações (4.3)** (vide arquivo [4.3.ParticipacoesArqReutilizacao.md](4.3.ParticipacoesArqReutilizacao.md))
